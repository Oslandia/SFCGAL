...
BOOST_AUTO_TEST_CASE( testFileUnionTest )
{
    ...
    while ( std::getline( ifs, line ) ) {
        ...
        try {
            if ( dimension == "2" ) {
                std::auto_ptr<Geometry> result = algorithm::union_( *gA,*gB );
                BOOST_REQUIRE( result.get() );
                BOOST_CHECK_MESSAGE( *result == *gOut, numLine << ": union_(" << gA->asText() << ", " << gB->asText() << ") is " << result->asText() << " and should be " << gOut->asText() );
            }
            else if ( dimension == "3" ) {
                std::auto_ptr<Geometry> result = algorithm::union3D( *gA,*gB );
                BOOST_REQUIRE( result.get() );
                BOOST_CHECK_MESSAGE( *result == *gOut, numLine << ": union3D(" << gA->asText() << ", " << gB->asText() << ") is " << result->asText() << " and should be " << gOut->asText() );
            }
            else {
                BOOST_CHECK( false );
            }
