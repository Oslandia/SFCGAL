...
BOOST_AUTO_TEST_CASE( testEmpty )
{
	...
	BOOST_CHECK( isNaN( detail::x( empty ) ) );
	BOOST_CHECK( isNaN( detail::y( empty ) ) );
	BOOST_CHECK( isNaN( detail::z( empty ) ) );
}
...
BOOST_AUTO_TEST_CASE( test2D )
{
	...
	BOOST_CHECK_EQUAL( detail::x( empty ), 3.0 );
	BOOST_CHECK_EQUAL( detail::y( empty ), 4.0 );
	BOOST_CHECK( isNaN( detail::z( empty ) ) );
}
...
BOOST_AUTO_TEST_CASE( test3D )
{
	...
	BOOST_CHECK_EQUAL( detail::x( empty ), 3.0 );
	BOOST_CHECK_EQUAL( detail::y( empty ), 4.0 );
	BOOST_CHECK_EQUAL( detail::z( empty ), 5.0 );
}
